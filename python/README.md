...
```
cd ultimateMRZ-SDK/binaries/<<os>>/<<arch>>
```
For example:
 ...
 From the the binaries folder (`ultimateMRZ-SDK/binaries/<<os>>/<<arch>>`), call the [setup.py](setup.py) script:
 ```
 python ../../../python/setup.py build_ext --inplace -v
 ```
 If you have multiple python versions installed, then you may use a virtual env, a full path to the executable... Also, you may use `python3` instead of `python` to make sure you're using version 3. The python version used to build the extension should be the same to use in order to run the samples.
 
 This will build and install the extension in the current folder. The extension is named **_ultimateMrzSdk.pyd** on Windows and **_ultimateMrzSdk.so** on Linux.
 
 Now you're ready to run Python scripts using the extension. We highly recommend checking the recognizer sample: [ultimateMRZ-SDK/samples/python/recognizer](../samples/python/recognizer)
