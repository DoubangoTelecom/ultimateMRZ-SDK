...
#if !defined(_ULTIMATE_MRZ_SDK_SAMPLES_UTILS_H_)
#define _ULTIMATE_MRZ_SDK_SAMPLES_UTILS_H_
...
* @returns 
*/
static bool mrzDecodeFile(const std::string& path, MrzFile& mrzFile)
{
	ULTMRZ_SDK_ASSERT(!path.empty());
...
#	endif
	if (!file) {
		ULTMRZ_SDK_PRINT_ERROR("Failed to open file at: %s", path.c_str());
		return false;
	}
...
static bool mrzParseArgs(int argc, char *argv[], std::map<std::string, std::string >& values)
{
...
#endif /* _ULTIMATE_MRZ_SDK_SAMPLES_UTILS_H_ */
